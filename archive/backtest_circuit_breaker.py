"""Portfolio Drawdown Circuit Breaker Backtest.

If the account's realized equity has fallen X% from its peak, block all new
entries until equity recovers. Existing open positions exit normally.

Thresholds tested: no breaker, -10%, -15%, -20% from peak equity.
...
def run(all_trades: dict, max_dd_trigger: float | None) -> dict:
    relevant = []
    ...
    for date_str in all_dates:
        ...
        # ── Circuit breaker: block entries if in drawdown beyond threshold ─────
        if max_dd_trigger is not None and current_dd >= max_dd_trigger:
            days_blocked += 1
            continue
...
def main() -> None:
    ...
    print("\nRunning circuit breaker comparison...")
    results = {label: run(all_trades, thr) for label, thr in THRESHOLDS.items()}
