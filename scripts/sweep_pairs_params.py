"""Pairs Trading parameter sweep.

Runs BacktestEngine over a grid of PairsTradingConfig parameters using locally
stored Parquet candle data. Results are saved to CSV and JSON.

Usage:
    python scripts/sweep_pairs_params.py
"""
...
_DEFAULT_SYMBOLS = ["AXISBANK", "ICICIBANK"]
...
PARAM_GRID: dict[str, list] = {
    "window_size": [120, 240, 360, 720],
    "entry_z_score": [2.0, 2.5, 3.0],
    "stop_loss_z_score": [3.0, 3.5, 4.0, 5.0],
}
...
def run_single(
    ...
    try:
        cfg = PairsTradingConfig(
            ...
            quantity_a=qty_a,
            quantity_b=qty_b,
            window_size=int(params.get("window_size", 360)),
            entry_z_score=float(params.get("entry_z_score", 2.5)),
            stop_loss_z_score=float(params.get("stop_loss_z_score", 4.0)),
        )
...
def save_results(results: list[dict], output_dir: Path) -> tuple[Path, Path]:
    ...
    csv_path = output_dir / "pairs_sweep_results.csv"
    json_path = output_dir / "pairs_sweep_results.json"
...
def print_top_results(results: list[dict], min_trades: int = 10) -> None:
